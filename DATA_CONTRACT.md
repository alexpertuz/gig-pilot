...
## Derived Local State (safe to rebuild)

These files are generated by the scanner, are gitignored, and may be deleted to
force a clean rebuild. They are not personal sources of truth and are never
modified by the system updater.

| File | Purpose |
|------|---------|
| `data/candidates.json` | Normalized full-content scan candidates keyed by URL |
| `data/triage.json` | Deterministic/model eligibility decisions, evidence, and cache fingerprints |
| `data/scores.json` | Fit scores for model-confirmed eligible gigs only |

The scanner writes these derived files before appending accepted URLs to the
User Layer. Corrupt or missing derived files fail closed: they must never cause
an unclassified item to appear as an active gig.
