#!/usr/bin/env node
/**
 * analyze-patterns.mjs — Rejection Pattern Detector for gig-pilot
 *
...
function analyze() {
  ...
  const enriched = entries.map(e => {
    ...
    const reportPath = reportMatch ? join(GIG_PILOT, reportMatch[1]) : null;
  ...
  if (geoBlocker && geoBlocker.percentage >= 20) {
    recommendations.push({
      action: `Tighten location filters in sources.yml -- ${geoBlocker.percentage}% of applications hit a geo-restriction blocker`,
