 ...
 * titles with roleFuzzyMatch from role-matcher.mjs, and flags any bucket+gig
 * that appears 2+ times with different URLs within a 90-day window. Such
 * clusters are almost certainly the same gig being re-listed by the poster —
 * a "still open / repeatedly posting" signal useful for prioritizing outreach.
 *
...
// links carry an author. Post permalinks (/r/<sub>/comments/...) do NOT, so
// this returns '' for them — the subreddit bucket handles those.
...
export function detectReposts(rows, windowDays = DEFAULT_WINDOW_DAYS) {
  ...
  const valid = rows
    ...
    .map(r => {
      ...
      // Grouping identity works for board gigs (company) AND demand-side Reddit
      // gigs (empty company): company || poster || subreddit || portal.
      const identity = company || posterFromUrl(r.url);
      const bucket = identity || redditBucketFromUrl(r.url) || r.portal || 'unknown';
      return {
        ...r,
        url: r.url.trim(),
        company,
  ...
  // Group by bucket (company || poster || subreddit || portal), case-insensitive.
  ...
  const clusters = [];
  for (const [, groupRows] of byBucket) {
...
function printSummary(clusters) {
  ...
  if (clusters.length === 0) {
    console.log('  No reposted gigs detected.\n');
    return;
  }

  const header =
    '  ' +
    'Poster/Source'.padEnd(22) +
    'Gig'.padEnd(34) +
    'Reposts'.padEnd(9) +
    'Span'.padEnd(12) +
    'First → Last';
  ...
  for (const c of clusters) {
    const company = (c.company || c.bucket || '').substring(0, 20).padEnd(22);
