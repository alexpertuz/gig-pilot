...
- `sources.yml` — Portal config (for filter update recommendations)

## Minimum Threshold

Before running analysis, check: does `data/leads.md` have at least 5 entries with status beyond "Evaluated" (i.e., Applied, Responded, Interview, Offer, Rejected, Discarded, SKIP)?
...
> - Update `sources.yml` to filter out geo-restricted roles
...
If the user agrees:
- For portal filter changes: edit `sources.yml`
