 ...
 * *acting on* it.
 *
 * gig-pilot is driven from an AI session, but there's no durable place to drop
...
const PATH = process.env.GIG_PILOT_INBOX || 'data/agent-inbox.md';

const HEADER = [
  '# Agent Inbox',
  '',
  '> **Agent protocol:** at the start of a gig-pilot session, read this file.',
...
function ensureGitignored() {
  ...
  if (process.env.GIG_PILOT_INBOX || PATH !== 'data/agent-inbox.md') return;
  try {
