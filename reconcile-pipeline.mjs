 ...
 * next `/gig-ops pipeline` run both re-surface them, and they get evaluated
...
const defaultPipeline = existsSync(join(GIG_OPS, 'data/pipeline.md'))
  ? join(GIG_OPS, 'data/pipeline.md')
  : join(GIG_OPS, 'pipeline.md');
const PIPELINE_FILE = resolveInsideRepo(argValue('--pipeline'), defaultPipeline, '--pipeline');
const STATE_FILE = resolveInsideRepo(argValue('--state'), join(GIG_OPS, 'batch/batch-state.tsv'), '--state');
...
for (let i = pendStart + 1; i < pendEnd; i++) {
  ...
  const reportLink = normalizeReportLink(`[${num}](reports/${reportFile})`, dirname(PIPELINE_FILE), GIG_OPS);
