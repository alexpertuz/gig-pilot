 ...
 * next `/gig-pilot pipeline` run both re-surface them, and they get evaluated
...
function resolveInsideRepo(inputPath, fallbackPath, flag) {
  ...
  try {
    repoReal = realpathSync(GIG_PILOT);
...
const defaultPipeline = existsSync(join(GIG_PILOT, 'data/pipeline.md'))
  ? join(GIG_PILOT, 'data/pipeline.md')
  : join(GIG_PILOT, 'pipeline.md');
const PIPELINE_FILE = resolveInsideRepo(argValue('--pipeline'), defaultPipeline, '--pipeline');
const STATE_FILE = resolveInsideRepo(argValue('--state'), join(GIG_PILOT, 'batch/batch-state.tsv'), '--state');
...
for (let i = pendStart + 1; i < pendEnd; i++) {
  ...
  const reportLink = normalizeReportLink(`[${num}](reports/${reportFile})`, dirname(PIPELINE_FILE), GIG_PILOT);
